...
export function LoginPage() {
  ...
  const { loginWithEmail, loginWithPhone, verifyOtp, loginWithSocial, isLoading } =
    useCustomerAuth();
  const { businessName, googleAuthEnabled, appleAuthEnabled } = useTheme();
  const socialEnabled = googleAuthEnabled || appleAuthEnabled;
  ...
  const tabs = [
    ...
    ...(socialEnabled ? [{ key: 'social' as Tab, label: 'Social', icon: Globe }] : []),
  ];
